...
-(void)formatImageView:(UIImageView*)imageView imageData:(id)data
{
    ...
    else if ([data isKindOfClass:[NSString class]])
    {
        ...
        if ([imageName hasPrefix:@"http"]) {
            //网络图片
            [imageView sd_setImageWithURL:[NSURL URLWithString:imageName] placeholderImage:[UIImage imageNamed:@"top_bg.png"]];
        }
        else
        {
            //本地图片
...
//        CGRect rect = CGRectMake(40, CGRectGetHeight(self.bounds)-20, CGRectGetWidth(self.bounds)-40*2, 10);
        CGRect rect = CGRectMake(CGRectGetWidth(self.bounds)-90, CGRectGetHeight(self.bounds)-20,40*2, 10);
        
        _pageControl = [[UIPageControl alloc]initWithFrame:rect];
        _pageControl.hidesForSinglePage = YES;
        _pageControl.pageIndicatorTintColor = [UIColor colorWithWhite:0.8 alpha:0.5];
        _pageControl.currentPageIndicatorTintColor = [UIColor whiteColor];
//        _pageControl.currentPageIndicatorTintColor = [UIColor colorWithRed:0/255.0f green:175/255.0f blue:240/255.0f alpha:1];

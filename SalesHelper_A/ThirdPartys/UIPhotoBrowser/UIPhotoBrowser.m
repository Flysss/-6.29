...
@interface UIPhotoBrowser () <UIPhotoViewDelegate>
{
    // 滚动的view
...
- (void)createToolbar
{
    ...
    _pageIndexLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, barY, self.view.frame.size.width, barHeight)];
    _pageIndexLabel.font = [UIFont boldSystemFontOfSize:20];
    ...
    [backBtn setImage:[UIImage imageNamed:@"首页-左箭头.png"] forState:UIControlStateNormal];
...
{
     _pageIndexLabel.text = [NSString stringWithFormat:@"%ld / %ld", (long)_currentPhotoIndex + 1, (long)_photos.count];
}
...
- (void)showPhotos
{
    ...
    for (UIPhotoView *photoView in _visiblePhotoViews) {
...
{
    UIPhotoView *photoView = [self dequeueReusablePhotoView];
    if (!photoView) { // 添加新的图片view
        photoView = [[UIPhotoView alloc] init];
...
//    photoViewFrame.origin.y = -20.0;
    photoViewFrame.origin.x = (bounds.size.width * index) + kPadding;
    photoView.tag = kPhotoViewTagOffset + index;
    
    UIPhoto *photo = _photos[index];
...
- (UIPhotoView *)dequeueReusablePhotoView
{
    UIPhotoView *photoView = [_reusablePhotoViews anyObject];
    if (photoView) {

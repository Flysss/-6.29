...
-(SCLAlertViewResponder *)showTitle:(UIViewController *)vc title:(NSString *)title subTitle:(NSString *)subTitle duration:(NSTimeInterval)duration completeText:(NSString *)completeText style:(SCLAlertViewStyle)style
{
    ...
    {
        case Success:
            viewColor = UIColorFromRGB(0x00aff0);
    ...
    if(completeText != nil)
    {
        SCLButton * button =[self addButton:completeText target:self selector:@selector(hideView)];
        button.tag = 110;
    ...
    [UIView animateWithDuration:0.2f animations:^{
        ...
        frame.origin.y = self.rootViewController.view.center.y + 50;
        ...
        [UIView animateWithDuration:0.2f animations:^{
            CGRect frame = self.view.frame;
            frame.origin.y = self.rootViewController.view.center.y/3;
            self.view.frame = frame;
        }];
    }];

...
- (void)configCollectionView {
    ...
    CGFloat margin = 4;
    CGFloat itemWH = (self.view.tz_width - 2 * margin - 4) / 3 - margin;
    ...
    _collectionView.contentSize = CGSizeMake(self.view.tz_width, ((_model.count + 3) / 3) * self.view.tz_width);
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    TZAssetModel *model;
    if (indexPath.row == 0) {
        cell.imageView.image = [UIImage imageNamed:@"拍照"];
        cell.type = TZAssetCellTypeCamera;

    } else {
        model = _photoArr[indexPath.row - 1];
        cell.model = model;
    }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary<NSString *,id> *)info
{
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:HWImagePickerImage object:nil userInfo:@{HWCameraImage : image}];

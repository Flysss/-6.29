...
-(void)setupSmallScrollViews{
    ...
    self.btnSave.titleLabel.font = [UIFont systemFontOfSize:11];
    [self.btnSave setTitleColor:[UIColor colorWithHexString:@"666666"] forState:(UIControlStateNormal)];
    self.btnSave.frame = CGRectMake(SCREEN_WIDTH/2 - 15, SCREEN_HEIGHT - 49, 30, 30);
    self.btnSave.backgroundColor = [UIColor whiteColor];
    self.btnSave.layer.cornerRadius = 15;
    self.btnSave.layer.masksToBounds = YES;
    [self.btnSave addTarget:self action:@selector(picSave:) forControlEvents:(UIControlEventTouchUpInside)];

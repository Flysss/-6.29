//
//  RecommendClientViewController.m
//  SalesHelper_A
//
...
@interface RecommendClientViewController () <UITableViewDelegate, UITableViewDataSource, UITextViewDelegate, recommendPropertyVCDelegate, UIAlertViewDelegate, UITextFieldDelegate>
...
@implementation RecommendClientViewController
{
    //客户手机
    UITextField *iphone;
    //客户姓名
...
- (void)viewDidLoad {
    ...
    //[self.info addObject:self.flotName];
    ...
    [requestHOp getInterfaceRequestObject:^(id data) {
        ...
        if ([data[@"success"] boolValue] == YES)
        {
            ...
            if (success)
            {
                ...
                [self.view endEditing:YES];
                [[NSNotificationCenter defaultCenter] postNotificationName:@"recomendSuccess" object:nil];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.section == 2 && indexPath.row == array.count)
    {
        ...
        __block RecommendClientViewController *test = self;
        vc.addBlick = ^(void){
            [test requestData];
        };

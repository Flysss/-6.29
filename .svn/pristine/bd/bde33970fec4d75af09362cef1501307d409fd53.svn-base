...
- (void)setModel:(BHFirstListModel *)model
{
    ...
    if (count == 1) {
        ...
        if ([model.imgwh isKindOfClass:[NSArray class]])
        {
            NSInteger height = [[model.imgwh lastObject] integerValue];
            NSInteger width = [[model.imgwh firstObject] integerValue];
            photoView.height = (self.width-10) * height / width;
            if (photoView.height > 235) {
                photoView.height = 235;
            }
            photoView.width = 235 * width / height;
            if (photoView.width > self.width-10) {
                photoView.width = self.width-10;
            }
            photoView.y = 10;
...
+ (CGFloat)HeightForView:(BHFirstListModel *)model
{
    ...
    if (count == 1)
    {
        if ([model.imgwh isKindOfClass:[NSArray class]])
        {
            NSInteger height = [[model.imgwh lastObject] integerValue];
            NSInteger width = [[model.imgwh firstObject] integerValue];
            height = height * (SCREEN_WIDTH-45) / width;
            if (height > 235) {
                height = 235;
            }
            return height+10;
...
- (void)setHmodel:(BHHuaTiModel *)Hmodel
{
    ...
    if (count == 1) {
        ...
        if ([Hmodel.imgwh isKindOfClass:[NSArray class]])
        {
            NSInteger height = [[Hmodel.imgwh lastObject] integerValue];
            NSInteger width = [[Hmodel.imgwh firstObject] integerValue];
            photoView.height = (self.width-10) * height / width;
            if (photoView.height > 235) {
                photoView.height = 235;
            }

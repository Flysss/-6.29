...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    else
    {
        ...
        if (indexPath.row == 1 && [cell.stageLabel.text rangeOfString:@"申请"].location != NSNotFound)
        {
       
            cell.confrimLabel.hidden = NO;
        }
        cell.lineLabel.frame = CGRectMake(37, 0, 0.5, height);
        cell.block = ^(UIButton* sender,NSString * string)
        {
            [self clickToCheckRejectImage:sender andString:string andIndexPath:indexPath];
        };
        return cell;
...
//查看拒绝图片
-(void)clickToCheckRejectImage:(UIButton*)sender andString:(NSString*)imageStr andIndexPath:(NSIndexPath*)indexPath
{
//    CGFloat  height = [RecommentClientCell tableViewCellHeightWithDic:m_dataArray[indexPath.row-1]];
//    RecommentClientCell * cell = [m_tableView cellForRowAtIndexPath:indexPath];
    
    NSArray * imageArr = [imageStr componentsSeparatedByString:@","];
    
//    NSLog(@"%@-%d",imageArr,[imageArr count]);
...
//        2.原始imageView
        photo.sourceImageView = [[UIImageView alloc]initWithFrame:CGRectMake(0,SCREEN_HEIGHT, SCREEN_WIDTH, 10)];
        //3.要放大图片的url
        photo.bigImgUrl =[NSString stringWithFormat:@"%@/%@",Image_Url,imageArr[i]];
        //标志
        photo.tag = i;
...
//    photoBrowser.currentIndex = (int)recognizer.view.tag ;
    photoBrowser.btnSave.hidden = YES;
    [photoBrowser show];
}
